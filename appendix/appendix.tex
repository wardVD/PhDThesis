...
\begin{equation}
    ...
    f(E) &= A\cdot E^{-\gamma} \rm\ \ \ (power law)\\
            ...
            \text{$E$ will follow from the distribution $f(E)$ as}\\
            E &= \left((1-u) \cdot E_{min}^{-\gamma+1} + u \cdot E_{max}^{-\gamma+1}\right)^{1/(-\gamma+1)}\\ 
            \end{split}
            \end{equation}
            For $\gamma$ = -1, we will make a uniform distribution in log space. This is shown in Fig. \ref{fig:powerlawrandom}.
            \begin{equation}
            \begin{split}
